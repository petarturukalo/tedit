...
void mv_left(cursor_t *crs, line_t *l)
{
	...
	if (i > 0) {
		...
		if (c == TAB_CONT) {
			...
			tab_ind = chrp_find_reverse(l->s_buf, TAB_START, 0, i-2);
...
{
	int prev_row, target_row = c->c_row - view_height(v);
...
void mv_view_pgdn(cursor_t *c, lines_t *ls, view_t *v)
{
	...
	if (target_row >= ls->sl_len)
		target_row = ls->sl_len-1;
	if (target_row != prev_row) {
		cursor_set_row(c, target_row, ls);
		v->v_pgmv = true;
	}
